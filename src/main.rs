...
async fn main() {
    ...
    let crd_api: Api<Directory> = Api::all(kubernetes_client.clone());
...
async fn reconcile(directory: Directory, context: Context<ContextData>) -> Result<ReconcilerAction, Error> {
    ...
    let namespace: String = match directory.namespace() {
        None => {
    ...
    return match determine_action(&directory) {
        Action::Create => {
            ...
            let name = directory.name(); // Name of the Echo resource is used to name the subresources as well.
            ...
            directory::deploy(client, &directory.name(), directory.spec.replicas, &namespace).await?;
            Ok(ReconcilerAction {
        ...
        Action::Delete => {
            ...
            directory::delete(client.clone(), &directory.name(), &namespace).await?;

            // Once the deployment is successfully removed, remove the finalizer to make it possible
            // for Kubernetes to delete the `Echo` resource.
            finalizer::delete(client, &directory.name(), &namespace).await?;
            Ok(ReconcilerAction {
...
fn determine_action(echo: &Directory) -> Action {
