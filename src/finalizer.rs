...
pub async fn add(client: Client, name: &str, namespace: &str) -> Result<Directory, Error> {
    let api: Api<Directory> = Api::namespaced(client, namespace);
    let finalizer: Value = json!({
        "metadata": {
            "finalizers": ["directories.example.com"]
        }
    });
...
///
/// Note: Does not check for resource's existence for simplicity.
pub async fn delete(client: Client, name: &str, namespace: &str) -> Result<Directory, Error> {
    let api: Api<Directory> = Api::namespaced(client, namespace);
    let finalizer: Value = json!({
        "metadata": {
            "finalizers": null
